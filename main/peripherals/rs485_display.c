...
volatile static int f_fade_output =0;
volatile static int  len_tx   = 0;
volatile static int  index_tx = 0;
volatile static unsigned long fade_output_timestamp = 0;
...
#define OUTPUT_ON() HAP_DIR_LAT = 0
#define OUTPUT_OFF() HAP_DIR_LAT = 1
...
void rs485_display_init(void) {
    ...
    HAP_RX_TRIS = SYSTEM_TRIS_INPUT;
    HAP_TX_TRIS = SYSTEM_TRIS_OUTPUT;
    HAP_DIR_TRIS = SYSTEM_TRIS_OUTPUT;
...
void rs485_display_manage(void) {
    if (f_fade_output && fade_output_timestamp != get_millis()) {
        OUTPUT_OFF();
        f_fade_output = 0;
    }
}
...
void __attribute__((interrupt, no_auto_psv)) _U1TXInterrupt(void)
{
    ...
    if (index_tx<len_tx) {
        ...
        f_fade_output = 1;
        fade_output_timestamp = get_millis();
        f_tx_on=0;
    }
